...
function fromDatabaseUrl(databaseUrl: string): PostgresConfig {
  const url = new URL(databaseUrl);

  if (!url.hostname || !url.username || !url.pathname.replace(/^\//, "")) {
    throw new Error(
      "DATABASE_URL must include host, user, and database name (postgresql://user:password@host:port/database)"
    );
  }
...
export function getPostgresConfig(): PostgresConfig {
  ...
  if (!parsed.success) {
    ...
    throw new Error(
      `Invalid PostgreSQL environment variables: ${details}. Set POSTGRES_HOST, POSTGRES_USER, POSTGRES_PASSWORD, and POSTGRES_DB, or provide DATABASE_URL.`
    );
  }
